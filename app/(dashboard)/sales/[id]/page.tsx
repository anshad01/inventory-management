...
export default async function SaleDetailPage({
  ...
  return (
    <div>
      ...
      <PageHeader title={`Sale ${sale.saleNumber}`}>
        <SaleActions id={sale.id} status={sale.status} />
      </PageHeader>
      ...
      <div className="mb-6 flex flex-wrap items-center gap-x-8 gap-y-2 text-sm">
        ...
        <div>
          ...
          <DocStatusBadge status={sale.status} />
        </div>
        <div>
      ...
      {sale.notes ? (
        <p className="mt-4 text-sm text-muted-foreground">Notes: {sale.notes}</p>
      ) : null}
    </div>
  );
}
